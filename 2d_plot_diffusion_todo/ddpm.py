...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        xt = (1 - alphas_prod_t).sqrt() * noise + alphas_prod_t.sqrt() * x0
        #######################
    ...
    def p_sample(self, xt, t):
        ...
        sigma_t = ((1 - alphas_prod_t_prev) * beta_t / (1 - alphas_prod_t)).sqrt()
        z = torch.randn_like(xt) if t > 1 else torch.zeros_like(xt)
        x_t_prev = (xt - eps_factor * eps_theta) / alpha_t.sqrt() + sigma_t * z
        #######################
        return x_t_prev
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        t, t_prev = t.to(xt.device), t_prev.to(xt.device)
        eps_theta = self.network(xt, t)

        beta_t = extract(self.var_scheduler.betas, t, xt)

        z = torch.randn_like(xt) if t > 1 else torch.zeros_like(xt)
        sigma_t = eta * ((1 - alpha_prod_t_prev) * beta_t / (1 - alpha_prod_t)).sqrt()

        mu_1 = alpha_prod_t_prev.sqrt() * ((xt - (1 - alpha_prod_t).sqrt() * eps_theta) / alpha_prod_t.sqrt())
        mu_2 = (1 - alpha_prod_t_prev - sigma_t * sigma_t).sqrt() * eps_theta

        x_t_prev = mu_1 + mu_2 + sigma_t * z
        ######################
        return x_t_prev
    ...
    def ddim_p_sample_loop(self, shape, num_inference_timesteps=50, eta=0.0):
        ...
        for t, t_prev in zip(timesteps, prev_timesteps):
            x = self.ddim_p_sample(x, t, t_prev, eta=eta)
        x0_pred = x
        ######################
